    ...
    return displayBoard.map((row, y) => (
      <div key={y} className="flex">
        {row.map((cell, x) => (
          <div
            key={x}
            className={`h-6 w-6 border border-gray-300 ${cell === "ghost" ? "bg-gray-500" : cell || "bg-gray-100"}`}
          />
        ))}
      </div>
    ));
    ...
    <div className="flex gap-4 p-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {gameOver && <div className="text-center font-bold text-red-600">Game Over!</div>}
